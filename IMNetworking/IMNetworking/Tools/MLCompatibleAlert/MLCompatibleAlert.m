                      ...
                      otherButtonTitles:(NSArray *)otherButtonTitles, ...
{
    ...
    if (self)
    {
        ...
        if ([[UIDevice currentDevice].systemVersion floatValue] >= 8.0)
        {
            ...
            __block id<MLCompatibleAlertDelegate> _block_delegate = delegate;
            ...
            for (NSString *t in titles)
            {
                UIAlertAction *action = [UIAlertAction actionWithTitle:t style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
                    if (_block_delegate && [_block_delegate respondsToSelector:@selector(compatibleAlert:clickedButtonAtIndex:)])
                    {
                        [_block_delegate compatibleAlert:self clickedButtonAtIndex:index];
                    }
                }];
            ...
            if (cancelButtonTitle != nil)
            {
                UIAlertAction *cancel = [UIAlertAction actionWithTitle:cancelButtonTitle style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
                    ...
                    else
                    {
                        if (_block_delegate && [_block_delegate respondsToSelector:@selector(compatibleAlert:clickedButtonAtIndex:)])
                        {
                            [_block_delegate compatibleAlert:self clickedButtonAtIndex:0];
                        }
                    }
                }];
                [_alertController addAction: cancel];
